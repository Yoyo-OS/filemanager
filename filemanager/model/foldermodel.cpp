...
void FolderModel::paste()
{
    ...
    if (enable) {
        ...
        if (mimeData->hasFormat("application/x-yoyo-cutselection")) {
...
void FolderModel::cut()
{
    ...
    mimeData->setData("application/x-yoyo-cutselection", QByteArray("1"));
...
{
    if (!url.isEmpty()) {
        QProcess::startDetached("yoyo-filemanager", QStringList() << url);
        return;
    }
    ...
    for (const QModelIndex &index : m_selectionModel->selectedIndexes()) {
        KFileItem item = itemForIndex(index);
        if (item.isDir()) {
            QProcess::startDetached("yoyo-filemanager", QStringList() << item.url().toLocalFile());
        }
    }
}
